...
git log --all --no-merges --pretty=format:"%h %s" -i -G "$VENDOR_RE" -- . ':(exclude)tools/scrub/*' || true
echo
echo
...
echo "--- summary ---"
n_commits=$(git log --all --no-merges --pretty=format:"%h" -G "$VENDOR_RE" -- . ':(exclude)tools/scrub/*' 2>/dev/null | wc -l | tr -d ' ')
