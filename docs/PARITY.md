...
| 11 | `bk guard stale-beads` | (no such command) | new subcommand: scans merged commit subjects on the default branch for bead-id tokens and reports open / in_progress beads whose work already shipped | bkg-bqa.3: post-merge sibling of `pr-beads`. Same Go-only-divergence rationale as `pr-beads` (CI-shaped feature; no Python counterpart). Match precision is the contract: subject-only + token-bounded + (conventional-scope OR `(#N)` PR-merge marker). Tangential references (`see <id> for context`) are not flagged. |
